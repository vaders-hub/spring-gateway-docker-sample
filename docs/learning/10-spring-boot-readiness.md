...
**Kubernetes가 Pod를 교체해 주더라도, 앱의 인증·설정·종료 처리가 올바르게 동작하는지는 별도 확인이 필요합니다.**
이번 단계는 4단계의 운영 실습을 Spring Boot 코드와 연결해, 설정 항목의 존재가 아니라 실제 효과를 검증하기 위해 진행합니다.
예를 들어 Pod 교체 후 같은 JWT가 유효한지 확인하고, 정상 rollout과 처리 중 요청의 안전한 종료를 구분합니다.
...
완료 기준은 현재 수행 가능한 인증 유지 실습의 결과를 남기고, 아직 구현되지 않은 종료·장애 검증은 준비 조건과 통과 기준으로 정리하는 것입니다.

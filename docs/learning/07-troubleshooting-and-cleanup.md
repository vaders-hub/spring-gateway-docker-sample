...
## 이 문서의 목적 / 왜 진단·종료·재개를 함께 배우나요?

실습 오류는 앱 코드뿐 아니라 WSL 접속 환경, Docker 엔진, 이미지, 포트, Pod 준비 상태에서도 발생합니다.
**재설치나 전체 삭제부터 하지 않고, 증거로 원인을 좁혀 필요한 대상만 복구하는 방법**을 익힙니다.
또한 정지와 삭제의 차이를 알아야 학습 데이터를 지키고 다른 프로젝트에 영향을 주지 않으면서 자원을 정리할 수 있습니다.
이 문서는 6단계 다음에 한 번 실행하는 과정이 아니라, **모든 단계에서 문제 발생·학습 종료·다음 재개 시 찾아보는 안내서**입니다.
완료 기준은 문제의 계층과 근거를 기록하고, 보존·삭제 대상을 구분해 의도한 리소스만 정리한 뒤 정상 상태로 재개하는 것입니다.

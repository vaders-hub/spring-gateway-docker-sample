...
## 이번 단계의 목적 / 왜 확장 계획을 먼저 세우나요?

로컬에서 배운 Kubernetes 구성을 **AWS에서 그대로 사용할 개념과 별도 설계·검증이 필요한 기능**으로 나눕니다.
kind에서 앱이 동작해도 AWS 권한·네트워크·저장소와 Oracle 트랜잭션까지 검증된 것은 아니기 때문입니다.
동시에 Spring Boot의 설정·보안·계층 구조를 읽고, Oracle/JPA/MyBatis 도입에 필요한 경계를 정리합니다.
이 단계는 AWS 리소스나 DB를 바로 생성하는 절차가 아니라 **역할 비교와 후속 구현 순서를 정하는 단계**입니다.
완료 기준은 현재 구현·실제 검증·미구현 과제를 구분하고, 다음에 수행할 작은 확장 과제와 검증 기준을 선택하는 것입니다.

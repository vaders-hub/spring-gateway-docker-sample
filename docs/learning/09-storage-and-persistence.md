...
**Pod는 교체될 수 있으므로, 앱 재기동과 데이터 보존을 별도로 설계해야 합니다.**
Docker 볼륨에서 배운 데이터 수명을 Kubernetes의 PV·PVC·StorageClass로 연결하고,
Pod 교체와 저장소 삭제가 데이터에 미치는 차이를 직접 확인하는 것이 이번 단계의 목적입니다.
PVC 사용만으로 백업이 갖춰지는 것은 아니므로 별도 사본으로 복원하는 과정까지 실습합니다.
완료 기준은 Pod 교체 후 보존 여부를 설명하고, PVC 삭제 후 새 PVC로 복원한 파일의 checksum이 원본과 일치함을 확인하는 것입니다.
이는 로컬 파일 저장 실습이며 실제 EBS 장애 복구나 Oracle DB 백업을 검증하는 단계는 아닙니다.
